...
export function NetworkView({owner, repo}: Props) {
  ...
  // Process commits into positioned nodes for VERTICAL graph
  const {positions, graphWidth, graphHeight, branchLabels, timelineLabels} = useMemo(() => {
    if (!network || network.commits.length === 0) {
      return {positions: [], graphWidth: 200, graphHeight: 200, branchLabels: [], timelineLabels: []}
    }

    const commits = network.commits // newest first (top to bottom)
    const commitMap = new Map<string, number>()
    commits.forEach((c, i) => commitMap.set(c.sha, i))

    // Assign lanes (x position) to each branch
    const branchLanes = new Map<string, number>()
    let laneIdx = 0

    // Default branch gets first lane (leftmost)
    ...
    // Track which lane each commit belongs to
    ...
    // Assign remaining commits to default branch lane
    commits.forEach(c => {
    ...
    // Build positioned commits (VERTICAL: x = lane, y = index)
    const positionedCommits: CommitPosition[] = commits.map((commit, idx) => {
      ...
      const color = BRANCH_COLORS[branchIdx >= 0 ? branchIdx % BRANCH_COLORS.length : 0]
      ...
      const parentPositions = commit.parentShas.map(parentSha => {
        const parentIdx = commitMap.get(parentSha)
        if (parentIdx === undefined) return null
        const parentLane = commitLanes.get(parentSha) ?? 0
        const parentBranchName = commitBranchName.get(parentSha) ?? network.defaultBranch
        const parentBranchIdx = network.branches.findIndex(b => b.name === parentBranchName)
        const parentColor = BRANCH_COLORS[parentBranchIdx >= 0 ? parentBranchIdx % BRANCH_COLORS.length : 0]
        return {
    ...
    // Create timeline labels (show date when it changes)
